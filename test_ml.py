if __name__ == "__main__":
    ...
    print('X_train={}, X_val={}, X_test={}'.format(
        X_train.shape, X_val.shape, X_test.shape))
    print('y_train={}, y_val={}, y_test={}'.format(
    ...
    trained_weights = logistic_regression_training(
        X_train, y_train, alpha=0.01, max_iters=1000, random_seed=1)
    print('trained_weights={}'.format(trained_weights))
    print('_______________________________________________________________________\n')

    trained_weights = logistic_regression_training(
        X_train, y_train, alpha=0.1, max_iters=500, random_seed=7171)
    print('trained_weights={}'.format(trained_weights))
    print('_______________________________________________________________________\n')

    # test logistic regression prediction
    train_preds = logistic_regression_prediction(
        X_train, trained_weights, threshold=0.5)
    print('train_preds={}'.format(train_preds[:10]))
    print('train accuracy={}'.format(
        (y_train.flatten() == train_preds.flatten()).sum()/y_train.shape[0]))
    print('_______________________________________________________________________\n')

    # test model selection and evaluation
    best_method, val_accuracy_list, test_accuracy = model_selection_and_evaluation(
        alpha=0.0001, max_iters=10, random_seed=321, threshold=0.7)
